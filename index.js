...
bot.on("ready", () => {
  ...
  bot.user.setActivity(`->help | Over ${bot.users.size} users!`, {type: 'WATCHING'})
  });
...
bot.elevation = function (msg) {
  ...
  let mod_role = msg.guild.roles.find("moderator", "Members");
  if (mod_role && msg.member.roles.has(mod_role.id)) permlvl = 2;

  let admin_role = msg.guild.roles.find("admin", "Higher-up Members");
  if (admin_role && msg.member.roles.has(admin_role.id)) permlvl = 3;

  if (msg.author.id === "554804147497598976") permlvl = 4;
  if (msg.author.id === "429336808405336076") permlvl = 4;
  if (msg.author.id === "399276720743514133") permlvl = 4;
  if (msg.author.id === "554804147497598976") permlvl = 4;
  if (msg.author.id === "554804147497598976") permlvl = 4;
  return permlvl;
};